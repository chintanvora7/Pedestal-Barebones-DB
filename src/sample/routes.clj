...
(defn add-db
  [db]
  (interceptor {:name  ::add-db
                :enter (fn [context] (assoc-in context [:request :db] db))}))
...
(defrecord Routes [routes graphstore db]
  component/Lifecycle
  (start [this]
    (prn "ROUTES DB: " db)
    (assoc this :routes (build-routes (:store this) db)))
  (stop [this] this))
